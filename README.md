...
2. **Runs TRIBE v2** on each segment: event extraction, predictions (~20,484 vertices per timestep), and optional transcription-related events.
...
| Requirement | Notes |
|-------------|--------|
| **GPU** | Strongly recommended; TRIBE v2 is GPU-heavy. The project was tested with **~48 GB VRAM** (e.g. RTX 6000 Pro on RunPod). |
...
`requirements.txt` installs `tribev2[plotting]` from GitHub, Gradio, the OpenAI client (for OpenRouter), NumPy/Pandas/Matplotlib, and `imageio[ffmpeg]` for GIFs.
...
| `brain.py` | TRIBE v2 loading, per-segment prediction, atlas aggregation, peaks/drops. |
...
TRIBE v2 is provided by Meta under its own terms; use of `facebook/tribev2` on Hugging Face is subject to that model’s license. This repository does not redistribute model weights.
